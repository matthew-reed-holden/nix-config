...
}:
{
  ...
  # Hyprland config — pattern 3, ported from EndeavourOS's autogenerated
  # default. Adds 1password autostart (SSH agent for git) and
  # hyprpolkitagent (sudo prompts in GUI). Mutable hyprctl tweaks
  # don't write back to this file, so the read-only symlink is fine.
